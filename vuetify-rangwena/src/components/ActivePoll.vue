  ...
  <div class="grid lg:grid-cols-2 gap-4">
    <v-card variant="text" rounded="0" elevation="0" density="compact">
      <v-card-text>
        ...
        <div class="mt-2 flex items-center">
          <span class="text-secondary" v-if="!!!voteStatus"
            ><v-btn
              loading
              variant="text"
              disabled
          ...
          <v-chip
            ...
            color="primary"
            v-else-if="voteStatus.voted === true"
          >
            <span>Voted</span>
          </v-chip>
          <div class="grow"></div>
    ...
    <div class="flex flex-col self-center">
      ...
      <v-card-text>
        <div
          class="mb-2"
          v-for="(votes, choice, index) in pollTally"
          :key="index"
        >
          ...
          >
            <span
              >{{ ((votes / poll.totalNumberOfvoters) * 100).toFixed() }}%
            </span>
          </v-progress-linear>
        </div>
        <div>
...
const pollTally = computed<Record<string, number>>(() => {
  if (tally.value) {
    return Object.entries(tally.value).reduce(
      (acc, [choiceId, votes]) => {
